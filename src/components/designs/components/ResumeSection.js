...
const Section = React.memo(({ title, children }) => (
  <div className="mb-6">
    <h3 className="text-xl font-semibold mb-2 border-b border-gray-300 pb-1">{title}</h3>
    {children}
  </div>
));

const ResumeSection = ({ resume, updateResumeData }) => {
  ...
  const handleStyleChange = useCallback(
    ...
    [selectedElement, resume, updateResumeData]
  );
  ...
  }, [selectedElement, selectedText, editedText, resume, updateResumeData]);
  ...
  const memoizedLeftColumn = useMemo(() => (
    ...
    />
  ), [resume, handleImageUpload, handleImageClick]);

  const memoizedRightColumn = useMemo(() => (
    <RightColumn resume={resume} />
  ), [resume]);

  if (!resume) return <div className="w-screen h-screen flex items-center justify-center">
...
const StyleOptions = memo(
  ({ editedText, handleTextChange, applyTextChange, handleStyleChange }) => (
    <div className="fixed top-0 right-0 bg-white p-4 shadow-md z-50">
      ...
      <div className="mb-2">
        ...
        <ColorPicker onChange={(color) => handleStyleChange("color", color)} />
      </div>
    </div>
  )
);

const LeftColumn = React.memo(({ resume, fileInputRef, handleImageUpload, handleImageClick }) => {
  const { personalInfo, Education, Contact, Projects } = resume;
    return (
...
const RightColumn = React.memo(({ resume }) => {
